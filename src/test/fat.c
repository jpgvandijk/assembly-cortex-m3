...
uint32_t FAT_AnalyseAndCreateSFN (char * name)
{
	...
	// N.B.: Special characters are not checked and must be valid for use in short file names!
...
uint32_t FAT_DirectoryFindNext (uint32_t type)
{
	...
	// N.B. same entry (without name) can be found again if Counter is reset to zero!
	...
	while (1)
	{
		...
		else
		{
			...
			if (entry->FileOtherInfo[0] == 0x0F)
			{
				...
				{
					// Check if start of LFN (checksum field will be ignored)
...
{
	// Return: 0 if error/not found, 1 otherwise. The file is specified using the FAT_DirectoryFind structure
		...
		else if (result == 1)
		{
			...
			else if (result == 2)
			{
				...
				else
				{
					...
					newCluster = FAT_DirectoryExtend(cluster, 1);
					if (newCluster == 0)
						return 0;
...
uint32_t FAT_DirectoryAddItem (uint32_t cluster, char * name, uint8_t type)
{
	...
	{
		// Allocate a cluster
		uint32_t newCluster = FAT_DirectoryExtend(0, 1);
		if (newCluster == 0)
			return 0;
...
/****** TRIALS: DIRTY, FAST IMPLEMENTATIONS ***********/

uint32_t FAT_DirectoryDeleteItem (void)
{
	// Return: 0 if error, 1 otherwise
	// The file is specified using the FAT_DirectoryFind structure
	uint32_t address = FAT_GetAddress(FAT_DirectoryFind.Cluster) + FAT_DirectoryFind.Sector;
	if (SDCARD_Read(address) == 0)
		return 0;
	FAT_DirectoryEntry_TypeDef * entry = ((FAT_DirectoryEntry_TypeDef *) SDCARD_ReadBuffer) + FAT_DirectoryFind.Entry;

	// Remember the starting cluster
	uint32_t cluster = FAT_DirectoryEntryGetStartCluster(entry);

	// TODO: delete the LFN entries! these can be (partly) in the previous cluster!

	// Write back with the entry deleted
	entry->FileShortName[0] = 0xE5;
	SDCARD_CopyReadBufferToWriteBuffer();
	if (SDCARD_Write(address) == 0)
		return 0;

	// Now free up the clusters/FAT entries IF actually any in use
	if (cluster == 0)
		return 1;

	// Get FAT sector containing first cluster
	if (FAT_Partition.Type == PARTITION_TYPE_FAT16)
	{
		address = FAT_Partition.AddressFAT1 + (cluster >> 8);
		if (SDCARD_Read(address) == 0)
			return 0;
	}
	else
	{
		address = FAT_Partition.AddressFAT1 + (cluster >> 7);
		if (SDCARD_Read(address) == 0)
			return 0;
	}

	// Remove all linked clusters from the FAT table
	while (1)
	{
		uint32_t temp;

		// Get next cluster, then free this cluster and advance
		if (FAT_Partition.Type == PARTITION_TYPE_FAT16)
		{
			temp = *(((uint16_t *) SDCARD_ReadBuffer) + (cluster & 0xFF));
			*(((uint16_t *) SDCARD_ReadBuffer) + (cluster & 0xFF)) = 0x0000;
			cluster = temp;
			temp = FAT_Partition.AddressFAT1 + (cluster >> 8);
		}
		else
		{
			temp = (*(((uint32_t *) SDCARD_ReadBuffer) + (cluster & 0x7F))) & 0x0FFFFFFF;
			*(((uint32_t *) SDCARD_ReadBuffer) + (cluster & 0x7F)) = 0x00000000;
			cluster = temp;
			temp = FAT_Partition.AddressFAT1 + (cluster >> 7);
		}

		// Only write back when new sector of FAT required
		if (temp != address)
		{
			// Write back to both FATs
			if (SDCARD_Write(address) == 0)
				return 0;
			if (SDCARD_Write(address + FAT_Partition.AddressFAT2 - FAT_Partition.AddressFAT1) == 0)
				return 0;

			// If last cluster, return
			if (cluster == 0xFFFF)
				return 1;
...
{
	// !!! Return: 0xFFFFFFFF on error, index in buffer otherwise

	// The file is specified using the FAT_DirectoryFind structure
	// Argument: byte offset from the starting cluster, if it would be bigger than the file the last sector is opened for appending,
	// unless at sector boundary then a sector is added to allow appending
	uint32_t address = FAT_GetAddress(FAT_DirectoryFind.Cluster) + FAT_DirectoryFind.Sector;
	if (SDCARD_Read(address) == 0)
		return 0xFFFFFFFF;
	FAT_DirectoryEntry_TypeDef * entry = ((FAT_DirectoryEntry_TypeDef *) SDCARD_ReadBuffer) + FAT_DirectoryFind.Entry;
	uint32_t startCluster = FAT_DirectoryEntryGetStartCluster(entry);
	uint32_t sector;

	// Append?
	if (offset >= entry->FileSize)
	{
		// Append, but sector might already be in use/available
		offset = entry->FileSize;
		if ((offset & (SDCARD_BLOCK_SIZE - 1)) == 0)
		{
			// New sector, no read required!
			sector = offset >> 9;

			// New cluster?
			if ((sector % FAT_Partition.SectorsPerCluster) == 0)
			{
				// Allocate a cluster (no clearing required)
				uint32_t newCluster = FAT_DirectoryExtend(startCluster, 0);
				if (newCluster == 0)
					return 0xFFFFFFFF;

				// Should we put it in the directory entry as start cluster?
				if (startCluster == 0)
				{
					// Read again, since read buffer changed by extend function
					if (SDCARD_Read(address) == 0)
						return 0xFFFFFFFF;
					FAT_DirectoryEntrySetStartCluster(entry, newCluster);
					SDCARD_CopyReadBufferToWriteBuffer();
					if (SDCARD_Write(address) == 0)
						return 0xFFFFFFFF;
				}
				...
				// Remember address
				FAT_DirectoryFind.Counter = FAT_GetAddress(newCluster) + 0;

				// No data to read in this sector since it was just added!
				return 0;
			}

			// Although no read required, fall through and read...
		}
	}

	// Read existing
	sector = offset >> 9;
	uint32_t cluster = sector / FAT_Partition.SectorsPerCluster;
	sector -= cluster * FAT_Partition.SectorsPerCluster;

	// Get the cluster
	while (cluster)
	{
		cluster--;
		startCluster = FAT_GetNextCluster(startCluster);
		if ((startCluster == 0) || (startCluster == 0xFFFFFFFF))
			return 0xFFFFFFFF;
	}

	// Remember address
	FAT_DirectoryFind.Counter = FAT_GetAddress(startCluster) + sector;

	// Get the data in the write buffer
	if (SDCARD_Read(FAT_DirectoryFind.Counter) == 0)
		return 0;
	SDCARD_CopyReadBufferToWriteBuffer();
	return (offset & (SDCARD_BLOCK_SIZE - 1));
}
...
uint32_t FAT_SaveFile (uint32_t n_bytes)
{
	...
	// FIXME:
	// n_bytes gives the number of bytes in the buffer that are meant for the file and should only be specified if appended, use 0 otherwise!
	// Note: can decrease file size, but only to the start of the sector!
	// The file is specified using the FAT_DirectoryFind structure, specific write region address is stored in the Counter variable.

	// First, write the data to free the write buffer for other use
	if (SDCARD_Write(FAT_DirectoryFind.Counter) == 0)
		return 0;

	// Update filesize if data appended
	if (n_bytes)
	{
		uint32_t address = FAT_GetAddress(FAT_DirectoryFind.Cluster) + FAT_DirectoryFind.Sector;
		if (SDCARD_Read(address) == 0)
			return 0;
		FAT_DirectoryEntry_TypeDef * entry = ((FAT_DirectoryEntry_TypeDef *) SDCARD_ReadBuffer) + FAT_DirectoryFind.Entry;
		entry->FileSize = (entry->FileSize & ~(SDCARD_BLOCK_SIZE - 1)) + n_bytes;
		SDCARD_CopyReadBufferToWriteBuffer();
		if (SDCARD_Write(address) == 0)
			return 0;
	}
